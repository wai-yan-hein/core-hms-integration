...
public interface OPDHisRepo extends JpaRepository<OPDHis, String> {
    ...
    @Query("select o from OPDHis o where o.vouPaid<>0 and o.intgUpdStatus is null and date(o.vouDate) >= :vou_date")
    List<OPDHis> unUploadVoucherCash(@Param("vou_date") Date syncDate);

    @Transactional
    @Modifying
    @Query("update OPDHis o set o.intgUpdStatus = :status where o.vouNo = :vouNo")
    void updateOPD(@Param("vouNo") String vouNo, @Param("status") String status);
